...
from formatters import fmt_one
...
def _start_add_user_convo(uid, msg_id):
    ...
    _safe_edit(uid, msg_id, prompt, reply_markup=menu.cancel_action(f"admin_manage_panel_hiddify"),parse_mode="MarkdownV2")
...
def _get_name_for_add_user(msg: types.Message):
    ...
    if name.startswith('/'):
        ...
        bot.send_message(uid, "عملیات ساخت کاربر لغو شد\\.", reply_markup=menu.admin_panel_management_menu('hiddify'))
        return
...
def _get_limit_for_add_user(msg: types.Message):
    ...
    try:
        ...
        _safe_edit(uid, msg_id, prompt, 
                   reply_markup=menu.cancel_action(f"admin_manage_panel_hiddify"),
...
def _get_mode_for_add_user(msg: types.Message):
    ...
    if choice not in mode_map:
        err_msg = escape_markdown("❌ انتخاب نامعتبر است. لطفاً عددی بین ۱ تا ۴ وارد کنید\\.")
        bot.send_message(uid, err_msg, parse_mode="MarkdownV2")
        bot.register_next_step_handler_by_chat_id(uid, _get_mode_for_add_user)
        return
...
def _finish_user_creation(uid, msg_id, mode):
    ...
    _safe_edit(uid, msg_id, escape_markdown(wait_msg_text), parse_mode="MarkdownV2")
